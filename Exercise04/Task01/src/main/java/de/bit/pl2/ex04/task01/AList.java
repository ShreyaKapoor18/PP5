...
class AList<Person> implements Iterable<Person> {
	private Person listArray[];                  // Array holding list elements
	...
	public AList(int size) {
		maxSize = size;
		 listSize = curr = 0;
		 listArray = (Person[])new Object[size];         // Create listArray
		}
	...
	public boolean insert(Person it) {
	...
	// Append "it" to list
	public boolean append(Person it) {
	 if (listSize >= maxSize) return false;
	...
	public Person remove() {
	 if ((curr<0) || (curr>=listSize))  // No current element
	   return null;
	 Person it = listArray[curr];            // Copy the element
